...
internal fun MaterialDialog.setWindowConstraints(fullScreenDialog: Boolean = false, fullWidth: Boolean = false, fullHeight: Boolean = false) {
    ...
    context.resources.apply {
        ...
        val maxWidth = if (fullScreenDialog || fullWidth) windowWidth - windowHorizontalPadding else getDimensionPixelSize(R.dimen.md_dialog_max_width)
        val calculatedWidth = windowWidth - windowHorizontalPadding

        this@setWindowConstraints.view.maxHeight = if (fullScreenDialog || fullHeight) windowHeight - windowVerticalPadding else windowHeight - windowVerticalPadding * 2
...
internal fun MaterialDialog.setTitleColor(
    ...
    //in case of header color
    // apply top margin else message will have no padding from header
    addContentScrollView()
    addContentMessageView(
            text = "",
            res = null)
    var windowVerticalPadding = 0
    context.resources.apply {
        windowVerticalPadding = getDimensionPixelSize(
                R.dimen.md_dialog_frame_margin_vertical_less
        )
    }
